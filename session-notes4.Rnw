...
q1.frame$obs =(1:length(q1.data))
 hline.data <- data.frame(z=c(c_regression_limits(mean(q1.data)),q1.trad.ucl),hue=c("darkblue","darkblue","blue"),ltype=c("dotted","dashed","dashed"),type=factor(c("regression","regression","tradtional")))
q1.base.plot =ggplot(hline.data) + geom_point(aes_string(x="obs",y="counts"),data=q1.frame) +
...
s -chart are preferred over R- charts if the computations are automated
\subsection{Work}
\subsection{Answer}
