...
\subsection*{answer}
 ...
 Compare the value of $n\bar{p}(1 -\bar{p}) $ to { \large$ \sum{ \frac{(X -\bar{X})^2}{n}} $}.
  These calculations should be in within a order of magnitude.
...
\subsection*{answer}
  The issue with $\bar{c}$ of 6.45 is the LCL will not exist. From our reading we know that if $\bar{c} < 9 $ then LCL will not exist because the LCL cannot leass than zero.  Using a transformation such as  $y_2=\sqrt{c} +\sqrt{c+1} $ creates a useful LCL.
